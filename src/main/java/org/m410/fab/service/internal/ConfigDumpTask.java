 ...
 * @author m410
 */
public class ConfigDumpTask implements Task {
    @Override
    ...
    public String getDescription() {
        return "Write out the configuration if changed";
    }
    ...
    public void execute(BuildContext context) throws Exception {
        ...
        context.setHash(getMD5Checksum(projectFile()));

        try(FileWriter writer = new FileWriter(projectConfigFile(context))) {
            yaml.dump(context, writer);
        }

    }

    private File projectConfigFile(BuildContext context) {
        return FileSystems.getDefault()
                .getPath(context.getBuild().getCacheDir(), "project.yml")
                .toFile();
    }
